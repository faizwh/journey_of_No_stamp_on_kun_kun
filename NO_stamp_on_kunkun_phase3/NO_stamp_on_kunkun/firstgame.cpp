...
void firstgame::initialize(){
    ...
    for(int i=0;i<5;++i){
        ...
        for(int j=0;j<6;++j){
            if(j==n){
                ...
                blocks[i*6+j]->setGeometry(270+((i*6+j)%6)*130,40+(i*6+j)/6*120,125,115);
    ...
    for(int j=0;j<6;++j){
        if(j==n){
            ...
            temp[j]->setGeometry(270+j*130,40-120,125,115);
            //label->setGeometry(x,y,width,height)(x,y)是左上角的坐标
            temp[j]->lower();
            temp[j]->show();
        }
...
void firstgame::move(){
    ...
    for(int j=0;j<6;++j){
        if(j==n){
            ...
            temp2[j]->setGeometry(270+j*130,40-120*2,125,115);
            //label->setGeometry(x,y,width,height)(x,y)是左上角的坐标
            temp2[j]->lower();
            temp2[j]->show();
        }
